...
public class EquipmentData : CardData
{
    ...
    /// <summary>
    /// 영웅 병종 타입 / 장비 병종 타입 / 옵션 병종 타입에 따른 장비 옵션 적용율 계산
    /// </summary>
    private eEquipmentOptionValueApplyRate GetEquipmentOptionValueApplyRateUnitType(
        eEquipmentApplyUnitType equipmentApplyUnitType, eEquipmentApplyUnitType optionApplyUnitType, eBattleUnitType unitType = eBattleUnitType.NONE)
    {
        eEquipmentOptionValueApplyRate applyRate = eEquipmentOptionValueApplyRate.DEFAULT;
        if (unitType != eBattleUnitType.NONE && unitType != eBattleUnitType.END)
        {
            switch (optionApplyUnitType)
            {
                case eEquipmentApplyUnitType.COMMON:
                    // 100%
                    break;
                case eEquipmentApplyUnitType.TROOP:
                    switch (unitType)
                    {
                        default:
                        case eBattleUnitType.WEAPON_SEIZE:
                        case eBattleUnitType.WEAPON_DEFENSE:
                        case eBattleUnitType.WEAPON_ANTI_PERSONNEL:
                            // 50%
                            applyRate = eEquipmentOptionValueApplyRate.RATE_50;
                            break;
                        case eBattleUnitType.TROOP_INFANTRY:
                        case eBattleUnitType.TROOP_ARCHER:
                        case eBattleUnitType.TROOP_CAVALRY:
                            // 100%
                            break;
                    }
                    break;
                case eEquipmentApplyUnitType.WEAPON:
                    switch (unitType)
                    {
                        default:
                        case eBattleUnitType.TROOP_INFANTRY:
                    ...
                    break;
                case eEquipmentApplyUnitType.INFANTRY:
                    // 100%
                    if (unitType == eBattleUnitType.TROOP_INFANTRY)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
                            applyRate = eEquipmentOptionValueApplyRate.RATE_150;
                        }
                    }
                    else
                    {
                    ...
                    break;
                case eEquipmentApplyUnitType.ARCHER:
                    // 100%
                    if (unitType == eBattleUnitType.TROOP_ARCHER)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
                    ...
                    break;
                case eEquipmentApplyUnitType.CAVALRY:
                    // 100%
                    if (unitType == eBattleUnitType.TROOP_CAVALRY)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
                    ...
                    break;
                case eEquipmentApplyUnitType.SEIZE:
                    // 100%
                    if (unitType == eBattleUnitType.WEAPON_SEIZE)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
                    ...
                    break;
                case eEquipmentApplyUnitType.DEFENSE:
                    // 100%
                    if (unitType == eBattleUnitType.WEAPON_DEFENSE)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
                    ...
                    break;
                case eEquipmentApplyUnitType.ANTI_PERSONNEL:
                    // 100%
                    if (unitType == eBattleUnitType.WEAPON_ANTI_PERSONNEL)
                    {
                        if (equipmentApplyUnitType == optionApplyUnitType)
                        {
                            // 150%
    ...
    /// <summary>
    /// EquipmentData -> CraftEquipmentServerInfo
    /// </summary>    
    public CraftEquipmentServerInfo ToCraftEquipmentServerInfo()
    {
        int mainOptionMetaID = 0, mainOptionValue = 0;
        int subOption1MetaID = 0, subOption1Value = 0;
        int subOption2MetaID = 0, subOption2Value = 0;
        int subOption3MetaID = 0, subOption3Value = 0;

        if (MainOption != null)
        {
            mainOptionMetaID = MainOption.MetaID;
            mainOptionValue = (int)MainOption.Value;
        }

        if (SubOption[0] != null)
        {
            subOption1MetaID = SubOption[0].MetaID;
            subOption1Value = (int)SubOption[0].Value;
        }

        if (SubOption[1] != null)
        {
            subOption2MetaID = SubOption[1].MetaID;
            subOption2Value = (int)SubOption[1].Value;
        }

        if (SubOption[2] != null)
        {
            subOption3MetaID = SubOption[2].MetaID;
            subOption3Value = (int)SubOption[2].Value;
        }

        int soulStoneMetaID = SoulStoneOption == null ? 0 : SoulStoneOption.MetaID;

        var data = new CraftEquipmentServerInfo(ObjectID, MetaID, Grade,
            mainOptionMetaID, mainOptionValue,
            subOption1MetaID, subOption1Value,
            subOption2MetaID, subOption2Value,
            subOption3MetaID, subOption3Value,
            soulStoneMetaID, EquipmentSetIndex);
    ...
    /// 뽑기 결과에서 등급 타입 리턴
    /// </summary>
    public eGachaResultGradeType GetGradeInGachaResult()
    {
        // 5티어 이상인 경우
        return Tier > eEquipmentTier.TIER_04 ? eGachaResultGradeType.GOOD : eGachaResultGradeType.NORMAL;
    }
}
